...
describe('Iterable', () => {
  ...
  test('reindex should set attributes correctly', async () => {
    document.body.innerHTML = '<table data-top-level="true" id="table" data-name="table">' +
      '<tr>' +
    ...
    expect(table.querySelector('#input-1-1').getAttribute('name')).
      toBe('table[0][input1]')
    expect(table.querySelector('#input-1-2').getAttribute('name')).
      toBe('table[0][input2]')

    expect(table.querySelector('#input-2-1').getAttribute('name')).
      toBe('table[1][input1]')
    expect(table.querySelector('#input-2-2').getAttribute('name')).
      toBe('table[1][input2]')
  })
})
